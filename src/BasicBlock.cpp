...
// dst：要插入的新指令，src：已经存在的目标指令，dst将插入在src前
void BasicBlock::insertBefore(Instruction *dst, Instruction *src)
{
    ...
    dst->setNext(src);// 步骤1: 新指令的next指向目标指令
    dst->setPrev(src->getPrev());// 步骤2: 新指令的prev指向目标指令的前一个指令
    src->getPrev()->setNext(dst);// 步骤3: 目标指令的前一个指令的next指向新指令
    src->setPrev(dst);// 步骤4: 目标指令的prev指向新指令

    dst->setParent(this);// 设置新指令的父节点为当前基本块
}
